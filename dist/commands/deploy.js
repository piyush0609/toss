...
async function wranglerJSON(cmd, env) {
    const { stdout } = await execAsync(cmd, env);
    return typeof stdout === 'string' ? stdout : stdout.toString();
}
async function createKV(title, env) {
    try {
...
async function createD1(name, env) {
    try {
...
export async function deployCommand() {
    ...
    await writeFile(join(workerDir, 'wrangler.toml'), `name = "${workerName}"
main = "src/index.ts"
