...
export async function doctorCommand() {
    ...
    ok = check(`Node.js ${nodeVersion}`, major >= 18, 'Upgrade to Node 18+ (https://nodejs.org)') && ok;
    // Wrangler installed
    let wranglerVersion = '';
    try {
    ...
    ok = check(wranglerVersion ? `Wrangler ${wranglerVersion}` : 'Wrangler not found', !!wranglerVersion && wranglerMajor >= 3, wranglerMajor < 3 ? 'Upgrade to Wrangler v3+ (npm install -g wrangler@latest)' : 'Run: npm install -g wrangler') && ok;
    ...
    ok = check(wranglerAuth ? `Authenticated as ${accountEmail}` : 'Not authenticated with Cloudflare', wranglerAuth, 'Run: wrangler login   (or export CLOUDFLARE_API_TOKEN=...)') && ok;
        ...
        try {
            ...
            if (accountMatch) {
                ...
                try {
                    ...
                    if (tokenMatch) {
                        const res = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountMatch[1]}/workers/subdomain`, { headers: { Authorization: `Bearer ${tokenMatch[1]}` } });
                        if (res.ok) {
    ...
    ok = check(subdomain ? `workers.dev subdomain: ${subdomain}` : 'No workers.dev subdomain registered', !!subdomain, 'Visit: https://dash.cloudflare.com/workers/onboarding') && ok;
    // Config
